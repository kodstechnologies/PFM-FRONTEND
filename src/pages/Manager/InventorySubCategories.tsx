...
//                                         <Tooltip title="Decrease"><IconButton size="small" onClick={() => {
//                                             const current = Number(p.quantity) || 0; const v = Math.max(0, current - 1);
//                                             // setLocalQuantity(p._id, v);
//                                             saveQuantity(p._id, v);
//                                         }} disabled={(Number(p.quantity) || 0) <= 0}><RemoveIcon /></IconButton></Tooltip>
//                                         <div className="flex items-center gap-1">
//                                             <TextField size="small" type="number" value={Number(p.quantity) || 0} onChange={(e) => {
...
//                                         </div>
//                                         <Tooltip title="Increase"><IconButton size="small" onClick={() => {
//                                             const current = Number(p.quantity) || 0; const v = current + 1;
//                                             // setLocalQuantity(p._id, v); 
//                                             saveQuantity(p._id, v);
...
const InventorySubCategories: React.FC = () => {
    ...
    const buildApiUrl = useCallback(
        (endpoint: string, ...pathParts: (string | null | undefined)[]): string => {
            let url = endpoint;
            if (url.startsWith('http')) {
            ...
            const cleanParts = pathParts.filter((part): part is string => !!part && typeof part === 'string');
            if (cleanParts.length > 0) {
                const separator = url.endsWith('/') ? '' : '/';
                url += `${separator}${cleanParts.join('/')}`;
            }
            ...
            try {
                ...
                console.error('❌ Invalid API URL:', url, e);
                throw new Error(`Invalid API URL: ${url}`);
            }
        },
        []
    ...
    const handleAuthError = useCallback(
        (status: number, errorText: string) => {
            if (status === 401 || errorText.toLowerCase().includes('token')) {
                localStorage.removeItem('accessToken');
                localStorage.removeItem('managerUser');
                navigate('/manager/login', { replace: true });
                return true;
    ...
    const getAccessToken = (): string | null => {
        ...
        if (!token) {
            const managerRaw = localStorage.getItem('managerUser');
            if (managerRaw) {
                try {
                    const manager = JSON.parse(managerRaw);
                    token = manager.accessToken || manager.token || null;
                } catch { }
            }
        }
        return token;
    ...
    const getCurrentManagerId = (): string | null => {
        const managerRaw = localStorage.getItem('managerUser');
        if (!managerRaw) return null;
        try {
            const manager = JSON.parse(managerRaw);
            return manager._id || manager.id || null;
        } catch {
            return null;
        }
    };
    ...
    const getStoreId = (): string | null => {
        ...
        try {
            ...
            if (!storeId) {
                console.warn('⚠️ No store ID found in manager object', { manager });
            }
            return storeId;
        } catch (err) {
            console.error('Failed to parse managerUser for storeId', err);
            return null;
        }
    };

    const normalizeProducts = (data: TypePayload | null): TypePayload | null => {
        if (!data) return null;
        ...
        return {
            ...data,
            subCategories: (data.subCategories || []).map((p: any) => {
                const qArr = Array.isArray(p.quantity) ? p.quantity : [];
                const managerEntry = currentManagerId
                    ? qArr.find((q: any) => String(q.managerId) === String(currentManagerId))
                    : null;
                ...
                if (managerEntry) {
                    numericQty = unit === 'pieces' || unit === 'piece' ? managerEntry.count ?? 0 : managerEntry.kg ?? 0;
                } else if (qArr.length > 0) {
                    const first = qArr[0];
                    numericQty = unit === 'pieces' || unit === 'piece' ? first.count ?? 0 : first.kg ?? 0;
                }
    ...
    const saveQuantity = useCallback(
        async (productId: string, newQty: number) => {
            ...
            try {
                ...
                if (!storeID) {
                    setError('Cannot update inventory: Store ID not found. Please log in again.');
                    return;
                }

                const currentManagerId = getCurrentManagerId();
                if (!currentManagerId) throw new Error('Manager ID not found');
                ...
                const body = {
                    managerId: currentManagerId,
                    count: isPieces ? newQty : 0,
                    kg: isPieces ? 0 : newQty,
                };
                ...
                setTypeCat((prev) =>
                    prev
                        ? {
                            ...prev,
                            subCategories: prev.subCategories.map((p) =>
        ...
        [typeCat, buildApiUrl, handleAuthError]
    );
    ...
    return (
        <div className="p-6">
            <div className="flex items-center mb-6">
                <IconButton onClick={handleBackToTypes} className="mr-2 bg-gray-100 hover:bg-gray-200">
                    <ArrowBackIcon />
                </IconButton>
                <div>
                    <Typography variant="h4">Sub Categories</Typography>
                    <Typography variant="body2" color="textSecondary">
                        Manage inventory for {typeCat.name}
                    </Typography>
                </div>
            </div>
            ...
            >
                {typeCat.subCategories.map((p) => {
                    ...
                    const basePrice = Number(p.price) || 0;
                    const finalPrice =
                        p.discount && basePrice > 0
                            ? Number(p.discountPrice) > 0
                                ? Number(p.discountPrice)
                                : basePrice - basePrice * (Number(p.discount) / 100)
                            : basePrice;

                    const unit = (p.unit || 'kg').toLowerCase();
                    const qty = Number(p.quantity) || 0;
                    const inputUnit = unit === 'pieces' || unit === 'piece' ? 'pcs' : 'kg';
                    const currentDisplay = `${qty} ${inputUnit}`;
                        ...
                        <Card key={p._id} className="hover:shadow-md transition-shadow">
                            <CardContent>
                                <div className="flex items-start gap-3">
                                    <Avatar variant="rounded" src={p.img || ''} alt={p.name} sx={{ width: 72, height: 72 }} />
                                    <div className="flex-1 min-w-0">
                                        <Typography variant="subtitle1" className="truncate max-w-[14rem]">
                                            {p.name}
                                        </Typography>
                                        <div className="mt-1 flex flex-wrap gap-1">
                                        ...
                                        <div className="mt-2">
                                            ...
                                            {p.discount > 0 && basePrice > finalPrice && (
                                                <Typography variant="caption" color="textSecondary" className="line-through">
                                                    ₹{basePrice.toFixed(0)}
                                                </Typography>
                                            )}
                                            <div className="mt-1">
                                ...
                                <div className="mt-4 flex items-center justify-between">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <Tooltip title="Increase">
                                            <IconButton
                                                size="small"
                                                onClick={() => saveQuantity(p._id, qty + 1)}
                                            >
                                                <AddIcon />
                                            </IconButton>
                                        </Tooltip>
                                    ...
                                    <div className="flex items-center gap-1">
                                        <Tooltip title="View Details">
                                            <IconButton size="small" color="info" onClick={() => handleViewDetails(p)}>
                                                <VisibilityIcon />
                                            </IconButton>
                                        </Tooltip>
                                        <Tooltip title="Save">
                                            <IconButton
                                                size="small"
                                                color="primary"
                                                onClick={() => saveQuantity(p._id, qty)}
                                ...
                                {(qty === 0 || qty < 10) && (
                                    <Box className="mt-3 flex items-center gap-2">
                                        <WarningIcon color={qty === 0 ? 'error' : 'warning'} fontSize="small" />
                                        <Typography variant="caption" color={qty === 0 ? 'error' : 'warning'}>
                                            {qty === 0 ? 'Out of stock' : 'Low stock — consider restocking'}
                                        </Typography>
                                    </Box>
                                )}
                            </CardContent>
                            ...
                            <div className="max-w-4xl mx-auto">
                                <div className="flex flex-col md:flex-row gap-6 mb-6">
                                    ...
                                    <div className="flex-grow">
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {normalizeTypeTokens(selectedProduct.type).map((type, i) => (
                                                <Chip key={i} label={type} variant="outlined" color="primary" />
                                            ))}
                                        </div>
