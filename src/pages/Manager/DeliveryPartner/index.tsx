...
const DeliveryPartnerList: React.FC = () => {
    ...
    const getManagerData = useCallback((): ManagerUser | null => {
        try {
    ...
    const getAuthToken = useCallback((): string | null => {
        ...
        if (!token) {
            ...
            if (managerUser && 'accessToken' in managerUser) {
                token = (managerUser as any).accessToken;
            }
        }
        return token;
    }, [getManagerData]);
    ...
    const fetchDeliveryPartners = useCallback(async () => {
        try {
            ...
            // Get manager data to extract managerId
            const managerData = getManagerData();
            if (!managerData) {
                setError('Manager data not found. Please log in again.');
                return;
            }
            ...
            if (!currentManagerId) {
                setError('Manager ID not found in manager data.');
                return;
            }
            ...
            console.log("🚀 ~ Fetching delivery partners with managerId:", currentManagerId);
            ...
            if (data.success && data.data) {
                ...
                // The API returns formattedPartners with name, phone, store
                const transformedPartners: DeliveryPartner[] = data.data.map((partner: any, index: number) => ({
                ...
                console.log('✅ Delivery partners fetched successfully:', transformedPartners);
            } else {
                setDeliveryPartners([]);
    ...
    }, [buildApiUrl, getManagerData, getAuthToken]);
    ...
    const handleDeletePartner = async (partnerId: string) => {
        if (window.confirm('Are you sure you want to delete this delivery partner?')) {
            try {
                ...
                // Include managerId in delete request if needed
                const queryParams = new URLSearchParams();
                if (managerId) {
                ...
                if (response.ok) {
                    ...
                    const errorText = await response.text();
                    throw new Error(`Failed to delete delivery partner: ${response.status} - ${errorText}`);
                }
            } catch (error) {
