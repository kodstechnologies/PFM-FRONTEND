...
const SubCategoriesEdit: React.FC = () => {
    ...
    const product = location.state?.subCategory as SubCategory | undefined;
    ...
    useEffect(() => {
        const fetchSubCategory = async () => {
            if (product) {
                console.log("🚀 ~ fetchSubCategory ~ Using passed product from state");
        ...
        fetchSubCategory();
    }, [subCategoryId, product, navigate]);
    ...
    const onSubmit: SubmitHandler<FormInputs> = async (data) => {
        ...
        try {
            ...
            toast.success(`Subcategory "${data.subCategoryName}" updated successfully!`, {
                ...
                position: 'top-right',
                autoClose: 2000,
                onClose: () => navigate(`/sub/categories/${typeId}`, { state: { typeId, categoryId } }),
            });
            reset();
            setTypes([]);
            setPreview(null);
    ...
    if ((!fetchedProduct && !product) || !subCategoryId) {
        return (
