...
//                 ),
//                 cell: (info) => (
//                     <span className="font-mono text-sm bg-gradient-to-r from-gray-50 to-gray-100 px-3 py-1 rounded-full text-gray-700 border border-gray-200">
//                         {info.getValue()}
//                     </span>
//                 ),
//                 size: 160,
...
//                 header: () => (
//                     <div className="flex items-center gap-2">
//                         <WorkIcon fontSize="small" className="text-gray-500" />
//                         <span>Role</span>
//                     </div>
//                 ),
//                 cell: (info) => {
...
//                     }
//                     const colorClass = roleColors[info.getValue()] || 'bg-gray-100 text-gray-800'
//                     return (
//                         <span className={`inline-flex px-2.5 py-1 text-xs font-semibold rounded-full ${colorClass} shadow-sm`}>
//                             {info.getValue()}
//                         </span>
//                     )
//                 },
...
//                         <span className="text-gray-900 font-medium">{info.getValue()?.name || 'N/A'}</span>
//                     </div>
//                 ),
//                 size: 200,
//                 enableSorting: true,
...
//                         <button
//                             onClick={() => handleView(row.original)}
//                             className="p-2 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-all duration-200 flex items-center shadow-sm hover:shadow-md"
//                             aria-label={`View ${row.original.firstName} ${row.original.lastName}`}
//                             title="View"
//                         >
//                             <VisibilityIcon fontSize="small" />
...
//                         <button
//                             onClick={() => handleEdit(row.original)}
//                             className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-all duration-200 flex items-center shadow-sm hover:shadow-md"
//                             aria-label={`Edit ${row.original.firstName} ${row.original.lastName}`}
//                             title="Edit"
//                         >
//                             <ModeEditIcon fontSize="small" />
...
//                         <button
//                             onClick={() => handleDelete(row.original)}
//                             className="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-all duration-200 flex items-center shadow-sm hover:shadow-md"
//                             aria-label={`Delete ${row.original.firstName} ${row.original.lastName}`}
//                             title="Delete"
//                         >
//                             <DeleteOutlineIcon fontSize="small" />
...
//     if (loading) {
//         return (
//             <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex justify-center items-center p-8">
...
//                 {/* Header Section */}
//                 <div className="mb-8">
//                     <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
//                         <div className="flex items-center gap-3">
...
//                             <input
//                                 type="text"
//                                 placeholder="Search by name, phone, role, ID, or store..."
//                                 value={globalFilter}
//                                 onChange={(e) => setGlobalFilter(e.target.value)}
//                                 className="w-full pl-12 pr-6 py-3 border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all duration-300 placeholder-gray-500 bg-white shadow-inner"
//                             />
//                         </div>
//                     </div>
...
//                                             <th
//                                                 key={header.id}
//                                                 className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-gray-50 transition-all duration-200 border-b-2 border-gray-100"
//                                                 style={{ width: header.getSize() }}
//                                                 onClick={header.column.getToggleSortingHandler()}
//                                             >
//                                                 <div className="flex items-center gap-2">
//                                                     {flexRender(header.column.columnDef.header, header.getContext())}
//                                                     {header.column.getCanSort() && (
//                                                         <span className={`ml-2 transition-transform ${header.column.getIsSorted() ? 'rotate-180' : ''}`}>
//                                                             {{
//                                                                 asc: "↑",
//                                                                 desc: "↓",
//                                                             }[header.column.getIsSorted() as string] ?? "↕"}
...
//                             <tbody className="bg-white divide-y divide-gray-100">
//                                 {table.getRowModel().rows.map((row) => (
//                                     <tr key={row.id} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
//                                         {row.getVisibleCells().map((cell) => (
...
//                                     <PersonIcon className="text-gray-400 text-6xl" />
//                                 </div>
//                                 <h3 className="mt-2 text-2xl font-bold text-gray-900">No employees found</h3>
//                                 <p className="mt-2 text-gray-600 text-lg">Get started by adding your first team member.</p>
//                                 <div className="mt-8">
//                                     <NavigateBtn
//                                         to="/employe/add"
//                                         label="Add New Employee"
...
//                     {/* Pagination */}
//                     <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-6 border-t border-gray-100 bg-gradient-to-r from-gray-50 to-blue-50">
...
//                                 {Math.min((pagination.pageIndex + 1) * pagination.pageSize, employeeData.filteredData.length)}
//                             </span>{" "}
//                             of <span className="text-blue-600">{employeeData.filteredData.length}</span> employees
//                         </div>
//                         <div className="flex items-center gap-2">
//                             <button
//                                 className="px-4 py-2 border-2 border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
//                                 onClick={() => table.previousPage()}
//                                 disabled={!table.getCanPreviousPage()}
//                             >
//                                 Previous
//                             </button>
//                             <div className="flex gap-1 bg-white px-2 py-1 rounded-xl shadow-inner">
//                                 {Array.from({ length: table.getPageCount() }, (_, i) => (
//                                     <button
//                                         key={i}
...
//                                     table.setPageSize(Number(e.target.value))
//                                 }}
//                                 className="px-4 py-2 border-2 border-gray-300 rounded-xl text-sm text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm"
//                             >
//                                 {PAGE_SIZES.map((pageSize) => (
//                                     <option key={pageSize} value={pageSize}>
//                                         Show {pageSize}
        ...
        () =>
            new Fuse(employeeData.allData, {
                keys: [
                    ...
                    "EmployeeeId",
                    { name: "store", getFn: (emp) => emp.store?.name || "" },
                    { name: "documentStatus.idProof", getFn: (emp) => emp.documentStatus?.idProof || "" },
                    { name: "documentStatus.addressProof", getFn: (emp) => emp.documentStatus?.addressProof || "" },
                    { name: "documentStatus.panProof", getFn: (emp) => emp.documentStatus?.panProof || "" },
                ],
                threshold: 0.3,
            }),
        [employeeData.allData],
    ...
    const columns = useMemo<ColumnDef<Employee, any>[]>(
        () => [
            ...
            columnHelper.accessor("documentStatus", {
                ...
                cell: (info) => {
                    const { idProof, addressProof, panProof } = info.getValue();
                    return (
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-500">ID:</span>
                                <span className={`px-1.5 py-0.5 rounded-full font-medium ${getStatusClass(idProof)}`}>
                                    {idProof}
                                </span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-500">Addr:</span>
                                <span className={`px-1.5 py-0.5 rounded-full font-medium ${getStatusClass(addressProof)}`}>
                                    {addressProof}
                                </span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-500">PAN:</span>
                                <span className={`px-1.5 py-0.5 rounded-full font-medium ${getStatusClass(panProof)}`}>
                                    {panProof}
                                </span>
                            </div>
                        </div>
                ...
                <div className="bg-white shadow-xl rounded-3xl border border-gray-200 overflow-hidden">
                    ...
                    <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-blue-50">
                        <div className="relative max-w-md">
                            ...
                            <input
                                type="text"
                                placeholder="Search by name, phone, role, ID, store, or document status..."
                                value={globalFilter}
                                onChange={(e) => setGlobalFilter(e.target.value)}
                                className="w-full pl-12 pr-6 py-3 border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all duration-300 placeholder-gray-500 bg-white shadow-inner"
                            />
