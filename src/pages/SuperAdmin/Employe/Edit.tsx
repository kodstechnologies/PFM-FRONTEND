...
const InputField: React.FC<InputFieldProps> = ({ label, id, name, type, value, onChange, required }) => (
...
const SelectField: React.FC<SelectFieldProps> = ({ label, id, name, value, onChange, options, required, statusColor, disabled = false }) => (
    <div className="relative">
        <label htmlFor={id} className="block text-sm font-medium text-gray-700 mb-2 flex items-center space-x-2">
            {/* <span className="text-xl flex-shrink-0">{icon}</span> */}
            <span>{label}</span>
